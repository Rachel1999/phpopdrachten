<?php
session_start();
?>

<!-- Side menu -->
<aside>
    <h2>Menu</h2>
    <ul>

        <!-- Opdrachten hoofdstuk 2 -->
        <li>Hoofdstuk 2
            <ul>
                <li>
                    <a href="/phpopdrachten/hoofdstuk2/opdracht_2-1.php">Opdracht 2.1</a>
                </li>
                <li>
                    <a href="/phpopdrachten/hoofdstuk2/opdracht_2-2.php">Opdracht 2.2</a>
                </li>
            </ul>
        </li>

        <!-- Opdrachten hoofdstuk 3 -->
        <li>Hoofdstuk 3
            <ul>
                <li>
                    <a href="/phpopdrachten/hoofdstuk3/opdracht_3-1.php">Opdracht 3.1</a>
                </li>
                <li>
                    <a href="/phpopdrachten/hoofdstuk3/opdracht_3-2.php">Opdracht 3.2</a>
                </li>
                <li>
                    <a href="/phpopdrachten/hoofdstuk3/opdracht_3-3.php">Opdracht 3.3</a>
                </li>
            </ul>
        </li>

        <!-- Opdrachten hoofdstuk 4 -->
        <li>Hoofdstuk 4
            <ul>
                <li>
                    <a href="/phpopdrachten/hoofdstuk4/opdracht_4-1.php">Opdracht 4.1</a>
                </li>
                <li>
                    <a href="/phpopdrachten/hoofdstuk4/opdracht_4-2.php">Opdracht 4.2</a>
                </li>
                <li>
                    <a href="/phpopdrachten/hoofdstuk4/opdracht_4-3.php">Opdracht 4.3</a>
                </li>
                <li>
                    <a href="/phpopdrachten/hoofdstuk4/opdracht_4-4.php">Opdracht 4.4</a>
                </li>
            </ul>
        </li>

        <!-- Opdrachten hoofdstuk 5 -->
        <li>Hoofdstuk 5
            <ul>
                <li>
                    <a href="/phpopdrachten/hoofdstuk5/opdracht_5-1.php">Opdracht 5.1</a>
                </li>
                <li>
                    <a href="/phpopdrachten/hoofdstuk5/opdracht_5-2.php">Opdracht 5.2</a>
                </li>
                <li>
                    <a href="/phpopdrachten/hoofdstuk5/opdracht_5-3.php">Opdracht 5.3</a>
                </li>
                <li>
                    <a href="/phpopdrachten/hoofdstuk5/opdracht_5-4.php">Opdracht 5.4</a>
                </li>
                <li>
                    <a href="/phpopdrachten/hoofdstuk5/opdracht_5-5.php">Opdracht 5.5</a>
                </li>
            </ul>
        </li>

        <!-- Instructies -->
        <li>Instructies
            <ul>
                <li>
                    <a href="/phpopdrachten/hoofdstuk2/Instructie/variables.php">Variabelen</a>
                </li>
                <li>
                    <a href="/phpopdrachten/hoofdstuk3/Instructie/if_else.php">If else</a>
                </li>
                <li>
                    <a href="/phpopdrachten/hoofdstuk4/Instructie/Dates.php">Dates</a>
                </li>
            </ul>
        </li>
    </ul>
</aside>
